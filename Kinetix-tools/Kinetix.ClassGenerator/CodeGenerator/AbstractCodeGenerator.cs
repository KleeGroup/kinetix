...
namespace Kinetix.ClassGenerator.CodeGenerator {
    ...
    public abstract class AbstractCodeGenerator {
        ...
        private TfsFileWriter _currentWriter;
                ...
                if (model.Namespaces != null && model.Namespaces.Values.Count > 0) {
                    foreach (ModelNamespace ns in model.Namespaces.Values) {
                        if (!Directory.Exists(ns.Name)) {
                            ...
                            string directoryForModelClass = GetDirectoryForModelClass(ns.HasPersistentClasses, model.Name, ns.Name);
                            string projectDirectory = GetDirectoryForProject(ns.HasPersistentClasses, model.Name, ns.Name);
                            string csprojFileName = Path.Combine(projectDirectory, model.Name + "." + ns.Name + ".csproj");
                            ...
                            foreach (ModelClass item in ns.ClassList) {
                                ...
                                using (_currentWriter = new TfsCsharpFileWriter(GetFilenameFromModelClass(item, model.Name), csprojFileName)) {
                                    ...
                                    if (!item.IsTable) {
                                        ...
                                        Console.Out.WriteLine("Skipping class " + ns.Name + "." + item.Name);
                                    }*/
                                }
                            }
                        }
        ...
        private void GenerateNotifyPropertyChangeField(ModelClass item) {
            ...
            if (item.PropertyList.Count > 0) {
                foreach (ModelProperty property in item.PropertyList) {
                    ...
                    ++propertyCount;
                    string dataType = CodeUtils.LoadShortDataType(property.DataType);
        ...
        private void GenerateDbContext(IEnumerable<ModelRoot> modelRootList) {
            ...
            using (_currentWriter = new TfsCsharpFileWriter(targetFileName)) {
        ...
        /*
        /// <summary>
        /// Génère le ResourceTypeLocator pour les classes.
        /// </summary>
        /// <param name="modelRootList">Liste des modèles.</param>
        private void GenerateResourceLocator(ICollection<ModelRoot> modelRootList) {
            Console.Out.WriteLine("Generating ResourceLocator");
            IEnumerator<ModelRoot> enumerator = modelRootList.GetEnumerator();
            enumerator.MoveNext();
            string projectName = enumerator.Current.Name;
            GenerateResXFile(projectName, modelRootList);
            GenerateDesignerFile(projectName);
        }

        /// <summary>
        /// Génère le fichier ResX.
        /// </summary>
        /// <param name="projectName">Nom du projet.</param>
        /// <param name="modelRootList">Liste des modèles objets associés.</param>
        private void GenerateResXFile(string projectName, ICollection<ModelRoot> modelRootList) {
            string fileName = Path.Combine(_outputDirectory, projectName + ".Dto\\ResourceLocator.resx");
            TfsFileWriter.CheckOutForEdit(fileName);
            using (ResXResourceWriter writer = new ResXResourceWriter(fileName)) {
                foreach (ModelRoot model in modelRootList) {
                    foreach (ModelNamespace ns in model.Namespaces.Values) {
                        foreach (ModelClass modelClass in ns.ClassList) {
                            foreach (ModelProperty property in modelClass.PropertyList) {
                                ResXDataNode node = new ResXDataNode(projectName + property.Class.Namespace.Name + property.Class.Name + property.Name, property.DataDescription.Libelle);
                                node.Comment = "Libellé de associé à " + projectName + "." + property.Class.Namespace.Name + "." + property.Class.Name + "." + property.Name;
        ...
        /// <summary>
        /// Génère le fichier .designer.cs associé au resx.
        /// </summary>
        /// <param name="projectName">Nom du projet.</param>
        private void GenerateDesignerFile(string projectName) {
            string fileName = Path.Combine(_outputDirectory, projectName + ".Dto\\ResourceLocator.resx");
            string designerFilePath = Path.Combine(_outputDirectory, projectName + ".Dto\\ResourceLocator.Designer.cs");
            TfsFileWriter.CheckOutForEdit(designerFilePath);
            string[] unmatchedElements;
            CodeDomProvider CodeProvider = new CSharpCodeProvider();
            CodeCompileUnit Code = StronglyTypedResourceBuilder.Create(fileName, "ResourceLocator", projectName + ".Dto", CodeProvider, false, out unmatchedElements);
            using (StreamWriter designerWriter = new StreamWriter(designerFilePath)) {
                CodeProvider.GenerateCodeFromCompileUnit(Code, designerWriter, new CodeGeneratorOptions());
        ...
        */
        ...
        private void GenerateReferenceAccessorsInterface(ICollection<ModelClass> classList, string projectName, string nameSpaceName) {
            string nameSpacePrefix = nameSpaceName.Replace("DataContract", string.Empty);
            ...
            string projectDir = Path.Combine(GetContractDirectoryName(projectName), nameSpaceContract);
            string csprojFileName = Path.Combine(projectDir, projectName + "." + nameSpaceContract + ".csproj");
            string interfaceFileName = Path.Combine(projectDir, "generated", interfaceName + ".cs");

            using (_currentWriter = new TfsCsharpFileWriter(interfaceFileName, csprojFileName)) {
                WriteLine(LoadUsing("System.Collections.Generic"));
        ...
        private void GenerateReferenceAccessorsImplementation(ICollection<ModelClass> classList, string projectName, string nameSpaceName) {
            ...
            string csprojFileName = Path.Combine(projectDir, projectName + "." + nameSpaceImplementation + ".csproj");
            Console.Out.WriteLine("Generating class " + implementationName + " implementing " + interfaceName);

            string implementationFileName = Path.Combine(projectDir, "Service.Implementation", "generated", implementationName + ".cs");
            using (_currentWriter = new TfsCsharpFileWriter(implementationFileName, csprojFileName)) {
                WriteLine(LoadUsing("System.Collections.Generic"));
                WriteLine(LoadUsing("System.Diagnostics.CodeAnalysis"));
                WriteLine(LoadUsing("System.ServiceModel"));
