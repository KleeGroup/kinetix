...
using Kinetix.Data.OracleClient;
#if NUnit
...
namespace Kinetix.Broker.Test {
    /// <summary>
    /// Classe de test du store Oracle.
    /// </summary>
    [TestFixture]
    public class OracleSqlStoreTest {
        /// <summary>
        ...
        public void TestInsertWithDoNothing() {
            using (TransactionScope tx = new TransactionScope(TransactionScopeOption.Required)) {
                ...
                Assert.AreEqual(1, store.Put(new Kinetix.Data.SqlClient.Test.Bean()));
                Kinetix.Data.SqlClient.Test.TestDbCommand command = Kinetix.Data.SqlClient.Test.TestDbCommand.LastCommand;
                Assert.IsTrue(command.CommandText.Contains("insert into BEAN(BEA_PK, BEA_LONG, BEA_SHORT, BEA_GUID, BEA_DOUBLE, BEA_DECIMAL, BEA_DATETIME, BEA_CHARS, BEA_CHAR, BEA_BYTES, BEA_BYTE, BEA_BOOL, BEA_INT, BEA_STRING) values (BEAN_SEQ.nextval, :BEA_LONG, :BEA_SHORT, :BEA_GUID, :BEA_DOUBLE, :BEA_DECIMAL, :BEA_DATETIME, :BEA_CHARS, :BEA_CHAR, :BEA_BYTES, :BEA_BYTE, :BEA_BOOL, :BEA_INT, :BEA_STRING) returning BEA_PK into :inserted_key"));
            }
        }
